...
class ACFS_Module extends Module
{
    ...
    public function acfef_repeater_field( $field )
    {
        ...
        /* acf_render_field_setting( $field, array(
        			'label'			=> __( 'Apply Limit to','acf' ),
        			'instructions'	=> '',
        			'type'			=> 'select',
        			'name'			=> 'edit_limit_role',
    ...
    public function acfef_exclude_groups( $field_group )
    {
        ...
        if ( !isset( $field_group['acfef_group'] ) ) {
            ...
            if ( isset( $current_screen->id ) && $current_screen->id == 'acfef_payment' ) {
                return $field_group;
            } else {
                return false;
            }
    ...
    public function acfef_render_email_field( $field )
    {
        ...
        if ( isset( $field['custom_email'] ) && $field['custom_email'] == 1 ) {
            ...
            if ( isset( $field['set_as_username'] ) && $field['set_as_username'] == 1 ) {
                echo  '<input type="hidden" name="set_as_username" value="' . $field['key'] . '"/>' ;
            }
        }
    ...
    public function acfef_fields_enqueue_scripts()
    {
        wp_enqueue_script(
            'acfef-fields',
            ACFEF_URL . 'acf-field-settings/fields/assets/js/fields.js',
            array( 'jquery' ),
            '4.0'
        );
    }
