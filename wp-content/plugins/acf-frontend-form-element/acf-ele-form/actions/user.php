...
class ActionUser extends ActionBase {
	...
	public function get_fields_display( $form_field, $local_field, $post_id = '' ){
		switch( $form_field[ 'field_type' ] ){
			case 'username':
				...
				$local_field[ 'disabled' ] = ! $form_field[ 'allow_edit' ];
			...
			case 'email':
				...
				$local_field[ 'set_as_username' ] = $form_field[ 'set_as_username' ] ;
			...
			case 'role':
				$roles = [];
				if( $form_field[ 'role_field_options' ] ){
				...
				$local_field[ 'default_value' ] = $form_field[ 'default_role' ];
	...
	public function action_controls( $widget, $step = false ){
		...
		$condition[ 'new_user_role!' ] = 'administrator'; 
		$widget->add_control(
			'hide_admin_bar',
			[
	...
	public function run( $post_id, $settings, $step = false ){
		...
		if( 'new_user' == $settings[ 'main_action' ] ){
			...
			if( isset( $_POST[ 'custom_email' ] ) ){ 
				...
				if( isset( $_POST[ 'set_as_username' ] ) ){
					$user_to_insert[ 'user_login' ] = $_POST[ 'acf' ][ $_POST[ 'set_as_username' ] ];	
				}
