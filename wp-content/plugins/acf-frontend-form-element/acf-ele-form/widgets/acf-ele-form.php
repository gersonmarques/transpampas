...
use  ACFFrontendForm\Module\Controls ;
...
class ACF_Elementor_Form_Widget extends Widget_Base
{
    ...
    protected function register_form_structure_controls()
    {
        ...
        $repeater->add_control( 'field_default_value', [
            ...
            'type'        => Controls_Manager::TEXT,
            'description' => __( 'This will populate a field if no value has been given yet.', 'acf-frontend-form-element' ),
            'dynamic'     => [
            'active' => true,
        ],
            'condition'   => [
    ...
    public function register_payment_controls()
    {
        $this->start_controls_section( 'payment_section', [
            'label' => __( 'Payment', 'acf-frontend-form-element' ),
            'tab'   => Controls_Manager::TAB_CONTENT,
        ] );
        $payment_option = [];
        if ( get_field( 'acfef_gateways', 'options' )['stripe_active'] ) {
            $payment_option['stripe'] = __( 'Stripe', 'acf-frontend-form-element' );
        }
        if ( get_field( 'acfef_gateways', 'options' )['paypal_active'] ) {
            $payment_option['paypal'] = __( 'Paypal', 'acf-frontend-form-element' );
        }
        $this->add_control( 'pay_action', [
            'label'     => __( 'Payment', 'acf-frontend-form-element' ),
            'type'      => Controls_Manager::SELECT2,
            'default'   => '',
            'multiple'  => true,
    ...
    public function get_form_fields( $settings, $wg_id, $form_args = array() )
    {
        ...
        foreach ( $settings['fields_selection'] as $key => $form_field ) {
            ...
            switch ( $form_field['field_type'] ) {
                ...
                default:
                    $local_field = array(
                        'name'          => $wg_id . $form_field['_id'] . '_' . $form_field['field_type'],
                        'key'           => $wg_id . $form_field['_id'] . '_' . $form_field['field_type'],
                        'label'         => '',
                        'wrapper'       => [
                        'class' => 'elementor-repeater-item-' . $form_field['_id'],
                    ],
    ...
    public function get_payment_options( $settings, $form_args )
    {
        return $form_args;
    }
    ...
    static function get_post_id( $settings, $form_args, $wg_id )
    {
        ...
        if ( 'new_post' == $settings['main_action'] || 'new_product' == $settings['main_action'] ) {
            ...
            if ( 'new_product' == $settings['main_action'] && class_exists( 'woocommerce' ) ) {
                ...
                $post_type = 'product';
                $post_status = $settings['new_product_status'];
        ...
        if ( 'edit_user' == $settings['main_action'] ) {
            ...
            if ( $settings['user_to_edit'] == 'url_query' && isset( $_GET[$settings['url_query_user']] ) ) {
    ...
    protected function render()
    {
        ...
        $submit_button .= '<input type="submit" class="acfef-submit-button acf-button button button-primary" value="%s" /><span class="acf-spinner"></span></div>';
        ...
        $form_args = $this->get_payment_options( $settings, $form_args );
        ...
        if ( $settings['fields_selection'] ) {
            $form_args['fields'] = $this->get_form_fields( $settings, $wg_id, $form_args );
        }
        ...
        if ( isset( $fields['hidden_data'] ) ) {
            $form_args['html_after_fields'] .= $fields['hidden_data'];
        }
        ...
        if ( $display ) {
            ...
            if ( isset( $_GET['modal'] ) && isset( $_GET['step'] ) && $_GET['step'] > 0 && $settings['show_in_modal'] ) {
                echo  '<script>
						jQuery(function() {
							jQuery( "div#modal_' . $_GET['modal'] . get_the_ID() . '" ).addClass("show").removeClass("hide");
						});
					</script>' ;
            }
        } else {
    ...
    {
        $scripts = [ 'jquery', 'acfef-frontend', 'acfef-ajax' ];
        return $scripts;
    }
